...
class VectorStore:
    ...
    def add_papers(self, papers: List[Dict]):
        ...
        for paper in papers:
            ...
            chunks = [
                html_content[i : i + 1000] for i in range(0, len(html_content), 1000)
            ]
...
class PaperAnalysisAgent:
    def __init__(self, vector_store: VectorStore):
        ...
        self.llm = ChatOpenAI(
            model="gpt-4-turbo-preview", api_key=environ.get("OPENAI_API_KEY")
        )
...
def main():
    ...
    parser.add_argument(
        "--prompts", nargs=4, required=True, help="Four search keywords"
    )
